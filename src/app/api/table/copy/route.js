...
// Handle copying tables to other portfolios
export async function PUT(request) {
    ...
    // Add new entry for the table/portfolio pair
    await knex("tables").insert({user: user.id, name: tableName, portfolio: portfolio})
        .onConflict().ignore();

    return NextResponse.json({message: "Table copied!"}, {status: 200});
}